...
export default function CalendarioPage(){
 ...
 async function carregar(){ const r=await v2Service.calendario({mes,ano}); setEventos(r.data?.eventos||[]) }
 useEffect(()=>{carregar().catch(()=>{})},[mes,ano])
 const porDia=eventos.reduce((acc,e)=>{const d=e.data?new Date(e.data).getDate():0;(acc[d]??=[]).push(e);return acc},{})
 const dias=new Date(ano,mes,0).getDate()
 return <div className="feature-page fade-in"><div className="page-header"><div><h1>Calendário Financeiro</h1><p className="page-desc">Receitas, despesas, parcelas, vencimentos de cartões, metas e recorrências em uma visão mensal.</p></div><button className="btn-save" onClick={carregar}><RefreshCw size={15}/> Atualizar</button></div>
